...
@njit
def diffusion_trial(drift_g, drift_s, boundary_g, beta, e_g, nb, C, e_s, SSD, boundary_s=0.25, dc=1.0, dt=.01, max_steps=2000):
    ...
    # unlumping C into b and p_s
    frac = np.random.uniform(0.02, 0.90).item()
    b = C * frac
    p_s = C * (1-frac)
...
def diffusion_condition(params, n_trials, SSDs):
    ...
    drift_g, drift_s, boundary_g, beta, e_g, nb, C, e_s = params
    choicert = np.empty(n_trials)
    for i in range(n_trials):
        choicert[i] = diffusion_trial(drift_g, drift_s, boundary_g, beta, e_g, nb, C, e_s, SSD = SSDs[i])
    return choicert
...
def batch_simulator(prior_samples, n_obs, dt=0.005, s=1.0, max_iter=1e4):
    ...
    # add VETs (e_g, e_s)
    for i in range(n_sim):
        sim_data_plus[:,:,4][i] = prior_samples[i, 4] # e_g
        sim_data_plus[:,:,5][i] = prior_samples[i, 7] # e_s
...
def prior(batch_size):
    ...
    while True:
        ...
        # 3. beta ~ U(0.25, 0.75)
        p_samples = np.random.uniform(low=(-2.0, 0.01, 0.1, 0.25),
                                      high=(2.0, 4.0, 2.0, 0.75), size=(batch_size, 4))

        # ndt_go = e_g + nb + b
        # 4. e_g ~ U(0.15, 0.25) //i.e., VET_go
        # 5. nb ~ U(0.01, 0.2)

        # C = b + p_s
        # 6. C ~ U(0.01, 0.3)
        ndt_go_comp = np.random.uniform(low = (0.15, 0.01, 0.01),
                                        high = (0.25, 0.2, 0.3), size = (batch_size, 3))
        p_samples = np.c_[p_samples, ndt_go_comp]

        # ndt_stop = e_s + p_s
        # 7. e_s ~ U(0.15, 0.25) //i.e., VET_stop
        ndt_stop_comp = np.random.uniform(low = 0.15,
                                          high = 0.25, size = (batch_size, 1))
...
trainer = ParameterEstimationTrainer(
    ...
    checkpoint_path="checkpoint/model_3" 
)
...
filename = "img/NDDM_fix_a_s_low_d_g_cut_no_stop/true_vs_est_epoch_1"
true_vs_estimated(plot_priors,
                  plot_param_means,
                  ['drift_g', 'drift_s', 'boundary_g', 'beta', 'e_g', 'nb', 'C', 'e_s'],
                  filename = filename,
                  figsize = (24,12),
                  dpi = 600,
                  show = False)
...
plt.title('drift_go')
drift_go_filename = 'img/NDDM_fix_a_s_low_d_g_cut_no_stop/drift_go_epoch_1'
...
drift_stop_filename = 'img/NDDM_fix_a_s_low_d_g_cut_no_stop/drift_stop_epoch_1'
...
boundary_go_filename = 'img/NDDM_fix_a_s_low_d_g_cut_no_stop/boundary_go_epoch_1'
...
for i in range(0, 100):
    ...
    plot_param_samples = amortizer.sample(plot_data, n_samples=1600)
    plot_param_means = plot_param_samples.mean(axis=0)
    filename = "img/NDDM_fix_a_s_low_d_g_cut_no_stop/true_vs_est_epoch_%s" % str((i+1)*10 + 1501)
    true_vs_estimated(plot_priors,
                      plot_param_means,
                      ['drift_g', 'drift_s', 'boundary_g', 'beta', 'e_g', 'nb', 'C', 'e_s'],
                      filename = filename,
    ...
    plt.ylabel('Posterior')
    plt.title('drift_go')
    drift_go_filename = 'img/NDDM_fix_a_s_low_d_g_cut_no_stop/drift_go_epoch_%s' % str((i+1)*10 + 1501)
    plt.savefig(drift_go_filename, dpi = 300)
    ...
    plt.ylabel('Posterior')
    plt.title('drift_stop')
    drift_stop_filename = 'img/NDDM_fix_a_s_low_d_g_cut_no_stop/drift_stop_epoch_%s' % str((i+1)*10 + 1501)
    plt.savefig(drift_stop_filename, dpi = 300)
    ...
    plt.ylabel('Posterior')
    plt.title('boundary_go')
    boundary_go_filename = 'img/NDDM_fix_a_s_low_d_g_cut_no_stop/boundary_go_epoch_%s' % str((i+1)*10 + 1501)
    plt.savefig(boundary_go_filename, dpi = 300)
...
#check priors for correlations
columns = ['drift_g', 'drift_s', 'boundary_g', 'beta', 'e_g', 'nb', 'C', 'e_s']

def plot_est1_vs_est2(par_array, par_names, par1, par2):
...
true_params = prior(100)
for i in range(0, len(columns)):
    for j in range(i+1, len(columns)):
        plot_est1_vs_est2(true_params, columns, columns[i], columns[j])
