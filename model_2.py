...
@njit
def diffusion_trial(drift_g, drift_s, boundary_g, boundary_s, beta, e_g, nb, C, e_s, SSD, dc=1.0, dt=.01, max_steps=2000):
...
def diffusion_condition(params, n_trials, SSDs):
    ...
    drift_g, drift_s, boundary_g, boundary_s, beta, e_g, nb, C, e_s = params
    choicert = np.empty(n_trials)
    for i in range(n_trials):
        choicert[i] = diffusion_trial(drift_g, drift_s, boundary_g, boundary_s, beta, e_g, nb, C, e_s, SSD = SSDs[i])
    return choicert
...
def prior(batch_size):
    ...
    while True:
        ...
        # 3. boundary_s ~ U(0.1, 0.25)
        # 4. beta ~ U(0.25, 0.75)
        p_samples = np.random.uniform(low=(-2.0, 0.1, 0.1, 0.1, 0.25),
...
# # check choiceRT distributions
# priors = prior(1)
# RTs = batch_simulator(priors, 5000000).astype(np.float32)
# RTs = RTs[:,:,0][0]
# RTs = np.array([i for i in RTs if i != 0])

# plt.figure()
# plt.hist(RTs, bins = 750)
# plt.show()
...
trainer = ParameterEstimationTrainer(
    ...
    checkpoint_path="checkpoint/model_2" 
)
...
# def stop_informative_prior(batch_size):
#     """
#     Samples from the prior 'batch_size' times.
#     ----------
...
#                                       high=(2.0, 4.0, 2.0, 0.25, 0.75), size=(batch_size, 5))
...
perc_succ = np.empty(100000)
for i in range(0,100000):
    true_params = stop_informative_prior(1)
    x = batch_simulator(true_params, 800).astype(np.float32)
...
true_vs_estimated(true_params[(np.all(param_means[:, list(range(5,9))] > 0.01, axis = 1)), :],
                  param_means[(np.all(param_means[:, list(range(5,9))] > 0.01, axis = 1)), :],
                  ['drift_g', 'drift_s', 'boundary_g', 'boundary_s', 'beta', 'e_g', 'nb', 'C', 'e_s'],
                  filename = "img/NDDM_lower_stop_boundary_lower_go_drift_cut_no-stopping")
...
columns = ['delta_g', 'delta_s', 'alpha_g', 'alpha_s', 'beta', 'e_g', 'nb', 'C', 'e_s']
for i in range(0, len(columns)):
    plt.figure()
    recovery(plot_param_samples[:, :, i].T.reshape(n_plot_points, n_trials, 1), plot_priors[:, i].squeeze())
    plt.xlabel('True')
    plt.ylabel('Posterior')
    plt.title(columns[i])
    drift_go_filename = 'img/results/two_bound/%s' % (str(columns[i]))
