...
# ====         ================                       ======================
# 30-Sep-21        Michael Nunez                  Converted from unidentified_DDM_BayesFlow.py
...
def prior(batch_size):
    ...
    n_parameters = 4
    p_samples = np.random.uniform(low=(-7.0, 0.1, 0.1, 0.0),
                                  high=(7.0, 3.0, 4.0, 1.0), size=(batch_size, n_parameters))
...
def diffusion_condition(params, n_trials):
    ...
    drift, boundary, ndt, beta = params
    choicert = np.empty(n_trials)
    for i in range(n_trials):
        choicert[i] = diffusion_trial(drift, boundary, ndt, beta)
        # choicert[i] = simulratcliff(drift, boundary, ndt, beta)  # Numba likes this line a certain way
    return choicert
...
true_vs_estimated(true_params, param_means, ['drift', 'boundary', 'ndt', 'beta'])
